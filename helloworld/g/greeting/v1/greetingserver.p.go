...
func Launch(ctx context.Context, sid id.ServiceId, impl Greeting) *future.Base[bool] {
	...
	ready.Handle(func (b bool) {
		...
		pcontext.Errorf(ctx,"Unable to start greeting.v1.Greeting, Ready returned false")
		pcontext.Dump(ctx)
...
func Init(ctx context.Context,require []lib.MustRequireFunc, impl Greeting) (*lib.ServiceMethodMap,*syscallguest.LaunchFuture, id.ServiceId){
	defer func() {
		if r := recover(); r != nil {
			pcontext.Infof(ctx, "InitGreeting: trapped a panic in the guest side: %v", r)
		}
		pcontext.Dump(ctx)
	}()
	...
	launchF.Failure(func (err syscall.KernelErr) {
		...
		pcontext.Errorf(ctx, "launch failure on call Greeting:%s",t)
...
func Run(ctx context.Context,
	...
	for {
		...
		if kerr == syscall.KernelErr_ReadOneTimeout {
			...
			pcontext.Infof(ctx,"calling backgrounder of Greeting")
...
func MustRegister(ctx context.Context) id.ServiceId {
    ...
    if err!=syscall.KernelErr_NoError {
        pcontext.Fatalf(ctx,"unable to register %s.%s","greeting.v1","greeting")
...
func MustRequire(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError {
        if err==syscall.KernelErr_DependencyCycle{
            pcontext.Errorf(ctx,"unable to require %s.%s because it creates a dependcy loop: %s","greeting.v1","greeting",syscall.KernelErr_name[int32(err)])
            panic("require greeting.v1.greeting creates a dependency loop")
        }
        pcontext.Errorf(ctx,"unable to require %s.%s:%s","greeting.v1","greeting",syscall.KernelErr_name[int32(err)])
...
func MustExport(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError{
        pcontext.Fatalf(ctx, "unable to export %s.%s","greeting.v1","greeting")
